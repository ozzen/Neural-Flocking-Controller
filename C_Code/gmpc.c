...
double fitness(State *s, Action *a, FitnessParameter *param, Constraint *c)
{
    ...
    // double f = param->wd * asd + 100000 * spC;
    // Obstacle_avoidance_final:
    // double f = param->wd * asd + 2000 * sp + 200000 * obst + 1000 * tgt;
...
double predatorAvoidance(State *s, double sqDistances[s->numBirds][s->numBirds], int numPreds, FitnessParameter *param, Constraint *c){
    ...
    double sp = seperationPolynomial(s, sqD, param);
    ...
    for(int k = n-numPreds; k < n; k++){
        for(int i = 0; i < n-numPreds; i++){
                ...
                // pa += abs(pow((c->predAvD - sqrt(sqDistances[i][k])),3));
                // spcube += 1.0f / pa;
        }
    }
    // for(int k = n-numPreds; k < n; k++){
...
int main(int argc, char *argv[])
{
    ...
    char *confFile = "C_Code/gmpc.conf";
    ...
    if (!quietMode) {
        ...
        printf("amax = %.3f \t\t vmax = %.3f \t\t alpha = %f\n", c.amax, c.vmax, c.alpha);
